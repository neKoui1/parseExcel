...
content=list()
for i in range(1,9):
    with open(f'./model_output/竞品分析prompt8_{i}.json','r', encoding='utf-8') as f:
        data = json.load(f)
        content.append(data)
    ...
    messages=[
        {
            ...
            请严格按照以下json格式输出，输出内容应包括所有的内容：
                ...
                我们的不足: "1. (不足1) 2. （不足2） ..."
                竞品的特点: ""
                聊天案例: "客户：（客户说的内容） 客服：（客服说的内容） 客户：（客户说的内容）"（直接放在一个字段中，不允许使用[]）
            ...
            请完整整合所有案例，确保无遗漏。pay attention: 输出的json格式应完全符合上述要求格式，包括标点符号
            '''
        }
    ]
)
...
with open(f'./model_output/竞品分析integrated.json', 'w', encoding='utf-8') as f:
    f.write(output)
